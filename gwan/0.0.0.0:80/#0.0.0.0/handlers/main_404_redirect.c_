...
int init(int argc, char *argv[])
{
   ...
   //  HDL_CLEANUP };
   u32 *states; get_env(argv, US_HANDLER_STATES, (char**)&states);
...
int main(int argc, char *argv[])
{
   ...
   u32 *pHTTP_status; get_env(argv, HTTP_CODE, (char**)&pHTTP_status);
   if(pHTTP_status && *pHTTP_status == 404) // is it a 404 error?
   {
      ...
      *pHTTP_status = 301; // setup 301:'moved permanently'
      ...
      if(!c)
      {
         ...
         char str[1024] = {0};
         char *wwwpath = (char*)get_env(argv, WWW_ROOT, 0); // get "/www" path
         s_snprintf(str, 1023, "%s/index.html", wwwpath);   // build full path
       	cacheadd(argv, str, 0, 0, 0, 200, 0); // never expire
       	c = cacheget(argv, "index.html", &len, 0, pHTTP_status, &mod, 0);
      }
      if(c)
      {   
         char *date = (char*)get_env(argv, SERVER_DATE, 0);
         char szmodified[32];
         static char buf[] = 
            "HTTP/1.1 %s\r\n"
            "Date: %s\r\n"
